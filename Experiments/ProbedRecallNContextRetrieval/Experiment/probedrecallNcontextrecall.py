...
class Experiment(object):
    def __init__(self):
        self.data_file = open('Data\\probedrecallNcontextrecall.dat', 'a')
    ...
    def _saveTrial(self, tInd, trial):
        ...
        output_line = '{}\t{}\t{}\t{}\n'.format(self.pID, self.session, tInd, output_line)
        print output_line
    ...
    def _setupPracticeTrials(self):
        ...
        serial_positions = range(1, self.exp_parameters.set_size+1)
        conditions = [(trial_type, serial_position) for serial_position in serial_positions for trial_type in trial_types for rep in range(self.exp_parameters.n_practice_rep)]
        numpy.random.shuffle(conditions)
        
        self.practice_trials = []

        for condition in conditions:
            trial_type, serial_position = condition
            if trial_type == 'content':
                probed_recall = myClasses.ProbedRecallTrial(self.avaliable_pool, self.exp_parameters, self.display)
                probed_recall.createProbe(serial_position)
                ...
                context_recall.createProbe(serial_position)
    ...
    def _setupExperimentTrials(self):
        trial_types = ['content', 'context']
        serial_positions = range(1, self.exp_parameters.set_size+1)
        conditions = [(trial_type, serial_position) for serial_position in serial_positions for trial_type in trial_types for rep in range(self.exp_parameters.n_experiment_rep)]
        numpy.random.shuffle(conditions)
    ...
    def _runExperiment(self):
        ...
        for tInd, trial in enumerate(self.experiment_trials):
            self.log('presenting trial {}.'.format(tInd))
            self.display.wait(1000)
            trial.displayTrial()
            self.recorder.showCursor()
            trial.getResponse(self.recorder)
